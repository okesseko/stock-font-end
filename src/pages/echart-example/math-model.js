...
function renderData() {
    fetch(`http://220.141.212.175:8080/api/display/tickRange?closedPrice=${final_price}`, {
        method: 'GET',
    })
    .then(response => {
        ...
        timer = setTimeout(function tick() {
            ...
            switch(kind) {
                ...
                case 'C':  
                    ...
                    .then(datas => {
                        console.log('order datas', datas)
                        let randomProperty = function (obj) {
                            let keys = Object.keys(obj);
                            return obj[keys[ keys.length * Math.random() << 0]];
                        };
                        console.log('randomProperty', randomProperty(datas.content))
                        quantity = getBatches();
                    
                        sendCancelApi({
                            "id": randomProperty(datas.content).id,
                            "quantity": quantity,
                        });        
                    });
                break;
